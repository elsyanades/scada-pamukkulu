...
<head>
  ...
  <title>Bangunan Bagi B.P.2</title>
                ...
                <div id="order_table">  
                     <table class="table table-bordered text-center">  
                          ...
                          <tr>  
                            ...
                            <?php
                            $url = 'http://localhost:8080/scada/conn_bp2.php';
...
function ExportToExcel(type, fn, dl) {
       ...
       return dl ?
         ...
         XLSX.writeFile(wb, fn || ('tma_bp2.' + (type || 'xlsx')));
    } 
 </script>  
 <script>
    function preview() {
...
document.getElementById("exportTableCSV").addEventListener("click", function(e){
    ...
    dl_anchor.download = "tma_bp2.csv";
  ...
  function pdf() {
            html2canvas(document.getElementById('order_table'), {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("tma_bp2.pdf");
                }
            });
        }
</script>
...
$('#myChart1').hide()
        ...
        async function getData() {
            const response = await fetch('http://localhost:8080/scada/conn_bp2.php');
...
<script>
    //  getDatas();
        async function getDatas() {
            const response = await fetch('http://localhost:8080/scada/data_bp2.json');
            // console.log(response);
            const data = await response.json();
            data.reverse();
            // console.log(data);
